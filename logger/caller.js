...
const FormatStackTrace = (err) => {
  ...
  for (let i = 0; i < trace.length; i++) {
    str +=
      ...
      trace[i].fileName +
      " at " +
      trace[i].functionName +
      " " +
      trace[i].lineNumber +
      ":" +
      trace[i].columnNumber +
      "\n";
  }
  return str;
};

const FetchCallerDetails = (err, idx) => {
  let trace = parse(err)[idx];
  return {
...
module.exports = { FormatStackTrace, FetchCallerDetails };
