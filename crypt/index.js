...
const key = process.env.NPM_ATOM_CRYPTO_PASSPHRASE;
...
const StringDecrypt = (data) => {
  let decData = crypto.AES.decrypt(data, keyutf, { iv: iv }).toString(
    crypto.enc.Utf8
  );
  return decData;
};
...
const ObjectEncrypt = (data) => {
  var encData = crypto.AES.encrypt(JSON.stringify(data), keyutf, {
    iv: iv,
  }).toString();
  return encData;
};
